...
object SevenSegmentSearchTests extends TestSuite:
  val tests = Tests {
    ...
    test("count 1s, 4s, 7s and 8s") - {
      ...
      val result = displays.map((display, outputs) =>
        outputs.map(output => display.toDigit(output.toList: _*))
      )

      assert(result.flatten.count(List(1, 4, 7, 8).contains) == 26)
      assert(result.map(_.mkString.toInt).sum == 61229)
    }

    test("full output for the initial example") - {
      val input = List(
    ...
    test("answer") - {
      ...
      assert(result.flatten.count(List(1, 4, 7, 8).contains) == 525)
      assert(result.map(_.mkString.toInt).sum == 1083859)
    }
  }
